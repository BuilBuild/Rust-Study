/*
 * @Author: LeiJiulong
 * @Date: 2025-07-21 20:57:03
 * @LastEditors: LeiJiulong && lei15557570906@outlook.com
 * @LastEditTime: 2025-07-21 21:26:30
 * @Description:
 */
...
pub async fn show_register_form(templ: web::Data<tera::Tera>) -> Result<HttpResponse, Error> {
...
pub async fn handle_register(
    ...
    let user = get_user_record(&app_state.db, &username);
    let user_not_found: bool = user.await.is_err();
    if user_not_found {
...
pub async fn handle_signin(
    ...
    if let Ok(user) = user {
        let does_password_match =
            argon2::verify_encoded(&user.user_password, params.password.clone().as_bytes())
                .unwrap();
